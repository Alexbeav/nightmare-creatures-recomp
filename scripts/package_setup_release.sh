...
#
# Writes: dist/nightmare-creatures-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix nightmare-creatures-recomp \
  --exe-name Nightmare_Creatures_Recompiled \
  --display-name "Nightmare Creatures Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Nightmare Creatures disc" \
